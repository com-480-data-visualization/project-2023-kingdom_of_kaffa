...
function setSlider(width, height, data_map){
  const data = Array.from({length: 7}, (_, i) =>new Date(1990 + i*5, 10, 3));
  ...
  const g = d3.select('#map_slider_figure1')
  .attr('width', width)
  .attr('height', height)
  .append('g')
...
function setMap(width, height, geojson) {
  ...
  const projection = d3.geoMercator().scale(170).center([0,20]).translate([width/2,height/2])

  const path = d3.geoPath().projection(projection);
  const color_legend = d3.scaleLinear()
        .domain([0, 3492660000])
        .range(['#eacaae', '#643100'])
...
function transitionMap(data, i) {
  ...
  const years = ["1990/91", "1995/96", "2000/01", "2005/06", "2010/11", "2015/16", "2019/20"] 
